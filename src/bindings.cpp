...
py::dict convert_sprite_atlas_result(vex::SpriteAtlasResult& sar) {
    ...
    auto blob_cap = py::capsule(blob_heap, [](void* p) {
        delete static_cast<std::vector<uint8_t>*>(p);
    });
    d["blob"] = py::array_t<uint8_t>(
        {static_cast<py::ssize_t>(blob_heap->size())}, {1},
    ...
    auto offs_cap = py::capsule(offs_heap, [](void* p) {
        delete static_cast<std::vector<int64_t>*>(p);
    });
    d["offsets"] = py::array_t<int64_t>(
        {static_cast<py::ssize_t>(offs_heap->size())}, {sizeof(int64_t)},
        offs_heap->data(), offs_cap);
...
py::tuple convert_results(std::vector<vex::LevelResult>& results, vex::DecodeMetrics& metrics) {
    ...
    for (auto& lr : results) {
        py::dict d;
        d["format"]    = (lr.format == vex::OutputFormat::JPEG_STREAM) ? "jpeg_stream" : "sprite_atlas";
        ...
        if (lr.jpeg_stream.has_value()) {
            ...
            for (auto& blob : jsr.blobs) {
                ...
                py::array_t<int64_t> offs(
                    {static_cast<py::ssize_t>(offs_heap->size())},
                    {sizeof(int64_t)},
                    offs_heap->data(), offs_cap);
                ...
                if (blob.data && blob.size > 0) {
                    ...
                    auto arr = py::array_t<uint8_t>(
                        {static_cast<py::ssize_t>(sz)}, {1}, ptr, cap);
...
PYBIND11_MODULE(_vex_core, m) {
    ...
    py::class_<vex::LevelConfig>(m, "LevelConfig")
        .def(py::init([](int width, int height, int quality,
                         const std::string& output, bool in_memory,
                         const std::string& cache_path, int atlas_columns) {
            vex::LevelConfig lc{};
            lc.width         = width;
            lc.height        = height;
            lc.quality       = quality;
    ...
    py::class_<vex::DecodeHandle, std::shared_ptr<vex::DecodeHandle>>(m, "DecodeHandle")
        .def("drain_events", [](vex::DecodeHandle& self) -> py::list {
        ...
        .def("peek_jpeg", [](vex::DecodeHandle& self, py::dict evt_dict) -> py::memoryview {
            vex::FrameEvent evt{};
            ...
            return py::memoryview::from_memory(
                static_cast<const void*>(data), static_cast<py::ssize_t>(out_size));
        })
        .def_property_readonly("progress", [](vex::DecodeHandle& self) -> py::dict {
        ...
        .def_property_readonly("done", [](vex::DecodeHandle& self) -> bool {
            return self.is_done();
        })
