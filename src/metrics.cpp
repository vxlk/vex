    ...
    int threads_used)
{
    ...
    for (const auto& tm : thread_metrics) {
        ...
        for (int l = 0; l < num_levels && l < static_cast<int>(tm.levels.size()); ++l) {
            ...
            if (src.width > 0)  dst.width  = src.width;
            if (src.height > 0) dst.height = src.height;
            if (src.quality > 0) dst.quality = src.quality;
            if (!src.output_format.empty()) dst.output_format = src.output_format;
        ...
        m.file_stats.insert(m.file_stats.end(),
                            tm.file_stats.begin(), tm.file_stats.end());
    }
