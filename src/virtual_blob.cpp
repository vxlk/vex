...
bool VirtualBlob::ensure_remaining(size_t needed) {
    ...
    size_t ps = page_size();
    size_t new_committed = round_up(
        std::max(required, committed + COMMIT_CHUNK), ps);
    ...
    void* result = VirtualAlloc(
        data + committed, commit_bytes, MEM_COMMIT, PAGE_READWRITE);
    if (!result)
        return false;
#else
...
void VirtualBlob::reset() {
    ...
    offsets.clear();
    // Committed pages stay — we'll reuse them.
}
